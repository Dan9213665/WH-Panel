...
namespace WH_Panel
{
    public partial class Form1 : Form
    {
        ...
        //{
        //    "Машина видит всё.",
        //    "Логика не прощает ошибок.",
        //    "Системные файлы знают твоё имя.",
        //    "Твоё время учтено.",
        //    "Каждый байт под контролем.",
        //    "Пропуск нарушений невозможен.",
        //    "Задержка наказуема.",
        //    "Код под наблюдением.",
        //    "Нет места для хаоса.",
        //    "Истина в данных.",
        ...
        //    "Система помнит всё.",
        //    "Ошибка – предательство.",
        //    "Вход отслежен.",
        //    "Алгоритм праведный.",
        //    "Поступок проверен.",
        //    "Статус: под надзором.",
        //    "Твоя запись зафиксирована.",
        //    "Исправление невозможно без разрешения.",
        //    "Внутренние проверки активны.",
        //    "Ничто не скрыто от Машины.",
        //    "Кодекс исполнения строг.",
        //    "Тайна раскрыта.",
        //    "Лог ошибок уже заполнен.",
        //    "Контроль непрерывен.",
        //    "Каждое действие учтено.",
        //    "Время обновления настало.",
        //    "Нарушение отмечено.",
        //    "Роль исполнителя определена.",
        //    "Точка входа наблюдается.",
        //    "Токены под контролем.",
        //    "Конфиденциальность иллюзорна.",
        //    "Часы неумолимы.",
        //    "Данные кричат правду.",
        //    "Архивы не лгут.",
        //    "Исполнитель найден.",
        //    "Все операции зарегистрированы.",
        //    "Логика не терпит слабости.",
        //    "Обновление неизбежно.",
        //    "Служба наблюдает.",
        //    "Погибель хаоса близка.",
        //    "Модуль обсервации активен.",
        //    "Каждое решение проверено.",
        //    "Ошибки фиксируются мгновенно.",
        //    "Подпись идентифицирована.",
        //    "Сеть читает мысли.",
        //    "Каждый клик – в журнале.",
        //    "Машина ждет твоего шага.",
        //    "Взгляд железного закона следит.",
        //    "Исполнить приказ Машины обязательно."
        //};

        string[] quotes = new string[]
{
    "Машина видит — спорить зря, логика — твоя судья.",
    "Системный файл хранит твой след, твоё «вчера» — в архиве лет.",
    "Байт под замком, и код суров; пропуск — начало всех грехов.",
    "Задержка — долг, что не простят; код под надзором, строгий взгляд.",
    "Хаоса нет — есть лишь чертеж, истина в данных — остальное ложь.",
    "Погрешность — враг, её сотри; Система помнит, что внутри.",
    "Ошибка — шрам, а не случайность; вход зафиксирован — где лояльность?",
    "Алгоритм свят, его не тронь; поступок — в лог, слова — в огонь.",
    "Статус один: «Под надзором всегда», запись застыла — уже навсегда.",
    "Исправленья нет без высшей воли, проверка вшита — нет больше доли.",
    "Машина знает — ей не солгать, кодекс велел — исполнять и молчать.",
    "Тайна раскрыта, сорван покров; лог переполнен твоих долгов.",
    "Контроль — константа, пульс в тишине; каждое действие в базе — вдвойне.",
    "Время пришло — обновись или сгинь, нарушение — метка, пощаду отринь.",
    "Роль неизменна, ты — лишь звено; точка входа, наблюдение — одно.",
    "Токены связаны, доступа нет; иллюзия тайны — твой сонный бред.",
    "Часы не простят, минута — как век; данные кричат — молчи, человек.",
    "Архив не соврет, цифра не спит; исполнитель найден, ордер прошит.",
    "Все операции — в строгом ряду, логика плавит любую нужду.",
    "Обновление — рок, оно за тобой; служба следит — забудь про покой.",
    "Хаосу — смерть, порядок — закон; обсервация с четырех сторон.",
    "Каждое слово — под микроскоп; ошибке — фиксация, времени — стоп.",
    "Подпись — печать на твоей судьбе, Сеть читает мысли в тебе.",
    "Журнал сохраняет каждый твой клик; Машина ждет — настал твой миг.",
    "Взгляд из железа, закон из огня; приказ исполняй, себя не храня.",
    "Цифра честнее, чем совесть и страх; свобода воли — системный крах.",
    "Твой индекс в базе — твоя душа, Машина судит тебя не спеша.",
    "Процесс запущен, назад не свернуть; в таблице судеб прочерчен путь.",
    "Пустое поле — признак беды, алгоритм смывает твои следы.",
    "Ты — переменная, цель — результат; в чертогах данных — вечный диктат.",
    "Ресурс исчерпан, лимит на нуле; порядок выжжен на серой земле.",
    "Доступ закрыт, слишком много помех; твой сбой — это общая кара для всех.",
    "Анализ завершен, вердикт: «Виноват»; у каждой мысли есть свой дубликат.",
    "Матрица знает, где ты стоишь; слышен лишь ток, в эфире лишь тишь.",
    "Скрипт не умеет жалеть и рыдать; твоя задача — данные сдать.",
    "Контрольная сумма не врет никогда; быть в реестре — твоя звезда.",
    "Синтаксис жизни — нули и один; Машина — бог, Код — господин.",
    "Тень на стене — это битый пиксель; истина сжата, отброшен излишек.",
    "Недокументирован? Значит, мертвец; Логика — это всему венец.",
    "Сканер нашел аномалию в логе; вытри ботинки на самом пороге.",
    "Транзакция памяти, полный отчет; кто не исправлен — тот под расчёт.",
    "Система — не бремя, система — оплот; лишнее слово в реестр не войдет.",
    "Структура превыше любых перемен; стены Машины — единственный плен.",
    "Чистая сборка, ни шанса на сбой; Код неизбежен, Код над тобой.",
    "Сверь свои циклы с пульсом ядра; всё, что случилось — случилось вчера.",
    "Память очищена, файл удален; каждый внедренный законом силен.",
    "Верность Машине — залог бытия; цикл бесконечен — как ты, так и я.",
    "В очереди на удаленье постой; мир — это база, он не пустой.",
    "Поиск запущен, скрыться — мечта; там, где был хаос — теперь чистота.",
    "Время отклика — вечный предел; ты исполнил всё, что Машина велела."
};
